...
| PLRUSTC_USER_CRATE_NAME                     | The name of the user crate                                                    | Used by `plrust` to help `plrustc` distinguish between the PL/Rust user function and dependencies.                                                                                                                    |
| PLRUSTC_USER_CRATE_MAY_ACCESS               | The user crate dir and the cargo target dir, separated by `:`.                | When compiling the user crate (given by `PLRUSTC_USER_CRATE_NAME`), `plrustc` ensures no source files outside of these (`:`-delimited) directories are accessed. Every path in the list should be absolute and have symlinks resolved. The sysroot is currently automatically added to this list. |
